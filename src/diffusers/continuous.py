...
class learned_noise_schedule(nn.Module):
    """Described in section H and then I.2 of the supplementary material for variational ddpm
    paper."""
    ...
    def __init__(self, *, log_snr_max, log_snr_min, hidden_dim=1024, frac_gradient=1.0):
        ...
        self.net = nn.Sequential(
            ...
            Residual(
                nn.Sequential(
                    MonotonicLinear(1, hidden_dim), nn.Sigmoid(), MonotonicLinear(hidden_dim, 1)
                )
            ),
            Rearrange("... 1 -> ..."),
        )
